...
class TopicModeler(object):
    ...
    def sort_by_distance(self, doc_index, doc_probs):
        ...
        # Append other features to doc_probs with a column_stack
        vectors = np.column_stack((doc_probs, self.other_features))
        # Use standard scaler on all vectors
        scaler = StandardScaler()
        doc_probs = scaler.fit_transform(vectors)
        # weigh features that are more important than others
        doc_probs = doc_probs[:,-2:] * 2
...
def main():
    ...
    tf_vectorizer = CountVectorizer(max_df=0.85, min_df=2, max_features=1000, stop_words='english')
    tm = TopicModeler(lda, tf_vectorizer)

    # tm.fit(df['beer_name'] + ' ' +  df['style'] + ' ' + df['brewery_name'], names=df['beer_name'])
    tm.fit(df['style'], names=df['beer_name'])
    tm.get_more_features(df)

    print(tm.top_topic_features())

    print(tm.top_closest_beers(df.beer_name.iloc[5],20))
    print(tm.top_closest_beers(df.beer_name.iloc[100],20))
    print(tm.top_closest_beers(df.beer_name.iloc[500],20))
    print(tm.top_closest_beers('Iron Mike Pale Ale',20))
